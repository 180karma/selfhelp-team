...
const AgentChatOutputSchema = z.object({
    response: z.string().describe("The AI agent's text response, which may include a task proposal."),
    question: z.object({
        text: z.string().describe("A follow-up question to the user. If a task is proposed, this should be a confirmation question."),
        options: z.array(z.string()).describe("A list of multiple-choice options for the user to select. If confirming a task, should be Yes/No style answers."),
        addTask: z.object({
    ...
    }).describe("A mandatory multiple-choice question to ask the user. If a task is proposed, this must be a confirmation to add the task."),
});
